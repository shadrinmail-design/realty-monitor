...
def extract_quantity(text):
    """Извлекает число из строки типа '24 помещения' или '15 машиномест'"""
    if not text:
        return None
    match = re.search(r'(\d+)', text)
    return int(match.group(1)) if match else None

def parse_akvilon_parking():
    """Парсит страницу паркингов Аквилон с количеством машиномест"""
        ...
        try:
            ...
            for card in cards:
                try:
                    ...
                    # Извлекаем количество машиномест (помещений)
                    ...
                    if name and link:
                        project = {
                            'name': f'Аквилон - {name}'.replace('\nСмотреть проект', '').strip(),
                            'url': link,
                            'price': price,
                            'quantity': quantity,
                            'quantity_text': quantity_text,
                            'found_at': datetime.now().isoformat(),
                            'source': 'akvilon_parking',
                            'type': 'parking'
                        }
            ...
            # 2. Убираем записи с названием типа "Паркинг X м²"
            filtered_projects = []
            for project in projects:
                ...
                # Проверяем название - исключаем паттерн "Паркинг X м²"
                name = project.get('name', '')
                if re.match(r'^(Аквилон - )?Паркинг\s+[\d.]+\s*м²', name):
                    continue

                filtered_projects.append(project)

            print(f'Найдено проектов с паркингами: {len(filtered_projects)}')
            for proj in filtered_projects:
                print(f"  {proj['name']}: {proj.get('quantity', 0)} машиномест")
...
if __name__ == '__main__':
    projects = parse_akvilon_parking()
    print(f'\nВсего найдено: {len(projects)} проектов')
