...
def format_weekly_report_message(all_sources_data):
    ...
    for source_name, data in all_sources_data.items():
        ...
        # Топ-5 проектов по количеству
        top_projects = sorted(data['projects'], key=lambda x: x.get('quantity', 0), reverse=True)[:5]
        for proj in top_projects:
            qty = proj.get('quantity', 0)
            name = proj.get('name', 'Без названия').replace('\nСмотреть проект', '')[:40]
            message += f'   • {name}: {qty} шт\n'
        
        if data['count'] > 5:
            message += f'   ... и еще {data["count"] - 5} проектов\n'
        
        message += '\n'
    
    message += f'<i>Следующий отчет: {(datetime.now()).strftime("%d.%m.%Y")}</i>'
...
def send_weekly_report():
    ...
    for source_id, parser_config in config.PARSERS.items():
        ...
        try:
            ...
            total_quantity = sum(p.get('quantity', 0) or 0 for p in projects)
